...
def compare_inverse_kinematic_solutions(robot, target_pose, q0):
    ...
    """
    # run each solver
    sol_lm = robot.ikine_LM(target_pose, q0=q0)
    sol_nr = robot.ikine_NR(target_pose, q0=q0)
    sol_gn = robot.ikine_GN(target_pose, q0=q0)
...
if __name__ == "__main__":
    ...
    for name, sol in solutions.items():
        print(f"{name} solution: {sol.q.round(4) if sol.success else 'No solution found'}")
    print("Errors:")
